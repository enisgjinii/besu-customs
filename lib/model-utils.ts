...
function categorizeMaterial(name: string): MaterialSection["category"] {
  ...
  if (lowerName.includes("panel") || lowerName.includes("door") || lowerName.includes("hood")) {
    return "Panels"
  }
  if (lowerName.includes("trim") || lowerName.includes("pipe") || lowerName.includes("edge")) {
    return "Piping/Trim"
  }
...
export function applyMaterialUpdates(scene: THREE.Group, sections: MaterialSection[]) {
...
export function getMeshByMaterialId(scene: THREE.Group, materialId: string): THREE.Mesh | null {
