...
const MODEL_PRESETS: Record<string, ModelPreset> = {
  ...
  'hoodie': {
    ...
    leftChest: {
      position: [0.70, 0.32, 0],  // Front left chest
      scale: [0.12, 0.12, 1],
      rotation: [0, 0, 0],
    },
    rightChest: {
  ...
  'tank': {
    ...
    leftChest: {
      position: [0.70, 0.30, 0],  // Front left chest
      scale: [0.10, 0.10, 1],
      rotation: [0, 0, 0],
    },
    rightChest: {
      position: [0.55, 0.30, 0],  // Front right chest
      scale: [0.10, 0.10, 1],
      rotation: [0, 0, 0],
    },
    back: {
...
export function getLogoPreset(
  ...
  placement: 'leftChest' | 'rightChest' | 'back' | 'leftSleeve' | 'rightSleeve' = 'leftChest'
): LogoPreset {
...
export function saveCustomPreset(
  ...
  console.log(`  preset: {`);
  console.log(`    position: [${preset.position.map(n => n.toFixed(4)).join(', ')}],`);
  console.log(`    scale: [${preset.scale.map(n => n.toFixed(2)).join(', ')}],`);
  console.log(`    rotation: [${preset.rotation.map(n => n.toFixed(2)).join(', ')}],`);
  console.log(`  }`);
}
