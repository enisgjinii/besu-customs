...
async function extractMaterialNames(modelPath) {
  try {
    ...
    const materialNameRegex =
      /"name"\s*:\s*"([^"]*?(?:Body_|Button_|Collar_|Sleeve_|Fabric_|Metal_|Zipper_|Elastic_)[^"]*?)"/gi;
    const matches = [];
    let match;
    ...
    console.error(
      "Error extracting materials from " + modelPath + ":",
      err.message,
    );
    return [];
  }
...
async function extractAllMaterialNames() {
  ...
  for (const modelFile of glbFiles) {
    try {
      ...
      const outputPath = path.join(
        outputDir,
        modelName + "-material-names-simple.txt",
      );
      const content =
        "Material Names for " +
        modelName +
      ...
      console.log(
        "  ✓ Saved " + materialNames.length + " materials to " + outputPath,
      );
    } catch (err) {
  ...
  const summaryContent =
    "All Material Names from 3D Models\n===============================\n\n" +
