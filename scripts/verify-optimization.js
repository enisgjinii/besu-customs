...
const models = fs.readdirSync(modelsDir).filter(f => f.endsWith('.glb') && !f.includes('-low') && !f.includes('-medium'));
let lowCount = 0;
let mediumCount = 0;
...
if (lodCheck) {
  console.log(`  ✅ Found ${lowCount} low and ${mediumCount} medium quality models`);
} else {
...
checks.push({
  ...
  details: swManagerExists ? 'service-worker-manager.ts exists' : 'Manager missing',
});
...
const indicatorPath = path.join(__dirname, '../components/connection-indicator.tsx');
const indicatorExists = fs.existsSync(indicatorPath);
checks.push({
  name: 'Connection Indicator',
  status: indicatorExists,
  details: indicatorExists ? 'connection-indicator.tsx exists' : 'Indicator missing',
});
...
const hasCaching = nextConfig.includes('Cache-Control') && nextConfig.includes('immutable');
checks.push({
...
if (totalOriginal > 0) {
  const lowSavings = ((totalOriginal - totalLow) / totalOriginal * 100).toFixed(1);
  const mediumSavings = ((totalOriginal - totalMedium) / totalOriginal * 100).toFixed(1);
  
  console.log(`  Original:  ${(totalOriginal / 1024 / 1024).toFixed(2)} MB`);
  console.log(`  Low:       ${(totalLow / 1024 / 1024).toFixed(2)} MB (${lowSavings}% smaller)`);
  console.log(`  Medium:    ${(totalMedium / 1024 / 1024).toFixed(2)} MB (${mediumSavings}% smaller)`);
}
