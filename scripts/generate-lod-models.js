  ...
  try {
    ...
    Object.entries(LOD_CONFIGS).forEach(([level, config]) => {
      const outputPath = path.join(
        outputDir,
        `${baseName}${config.suffix}.glb`,
      );

      // Skip if already exists
      if (fs.existsSync(outputPath)) {
        console.log(
          `  ⏭️  ${level.toUpperCase()} version already exists, skipping`,
        );
      ...
      try {
        ...
        console.log(
          `  ✅ ${level.toUpperCase()}: ${(outputSize / 1024).toFixed(2)} KB (${reduction}% smaller)`,
        );
      } catch (error) {
        console.error(
          `  ❌ Failed to generate ${level.toUpperCase()}:`,
          error.message,
        );
        stats.errors++;
      }
...
console.log(
  `  Original:  ${(stats.totalOriginalSize / 1024 / 1024).toFixed(2)} MB`,
);
console.log(
  `  Low:       ${(stats.totalLowSize / 1024 / 1024).toFixed(2)} MB (${((1 - stats.totalLowSize / stats.totalOriginalSize) * 100).toFixed(1)}% reduction)`,
);
console.log(
  `  Medium:    ${(stats.totalMediumSize / 1024 / 1024).toFixed(2)} MB (${((1 - stats.totalMediumSize / stats.totalOriginalSize) * 100).toFixed(1)}% reduction)`,
);
console.log("");
...
console.log(
  "  • Progressive loading: low → medium → high as connection allows",
);
console.log("");
