...
/**
 * Script to optimize the Duffle bag_01.glb file for faster loading
...
const inputFile = path.join(__dirname, '../public/models/Duffle bag_01.glb');
const outputFile = path.join(__dirname, '../public/models/Duffle bag_01_optimized.glb');
const backupFile = path.join(__dirname, '../public/models/Duffle bag_01_original.glb');

console.log('🚀 Starting Duffle Bag 01 optimization...');
...
try {
  ...
  execSync(
    `npx gltf-pipeline -i "${inputFile}" -o "${outputFile}" -d`,
    { stdio: 'inherit' }
  );
