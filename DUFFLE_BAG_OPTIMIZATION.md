# Backpack - Loading Speed Optimization

## Problem

The "Backpack.glb" file is **71MB**, which causes slow loading times.

## Solutions Implemented

### 1. Model Preloading ✅

Added `useGLTF.preload()` in `components/model-loader.tsx` to start loading the backpack model immediately when the app loads, before the user selects it.
...
```bash
npm run optimize-backpack
```

This will:

- Apply Draco compression to the model
- Backup the original file as `Backpack_original.glb`
...
- **Original size**: 71MB
- **Model path**: `/models/Backpack.glb`
- **Preloading**: Enabled
