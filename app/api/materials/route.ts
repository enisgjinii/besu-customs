...
export async function GET(request: Request) {
  ...
  if (!modelParam) {
    return NextResponse.json(
      { error: "model query param required" },
      { status: 400 },
    );
  }
  ...
  const renameSectionForModel = (originalName: string): string | null => {
    ...
    if (modelParam.includes("Baseball-Jersey.glb")) {
      ...
      if (originalName === "Default_Button_3683978")
    ...
    if (modelParam.includes("Polo shirts long sleeve.glb")) {
      ...
      if (originalName === "Body_F_18571006")
        return "Front of Polo Long Sleeve Shirt Color";
    ...
    if (modelParam.includes("Standard bottom cut, cuffed.glb")) {
      ...
      if (originalName === "Default_Button_46243")
        return "Button Stitching Color";
      if (originalName === "Default_Buttonhole_46302")
        return "Buttonhole Color";
    ...
    if (modelParam.includes("Volleyball shorts spandex 4.glb")) {
      ...
      if (originalName === "Default_Topstitch_1426725")
        return "Inseam of Shorts Stitching Color 2";
      if (originalName === "Default_Topstitch_1427150")
        return "Inseam of Shorts Stitching Color 3";
    ...
    // Backpack renaming
    if (modelParam.includes("Backpack.glb")) {
      if (originalName === "FABRIC_3_79203")
        return "Front of Backpack & Straps Color";
      if (originalName === "FABRIC_4_79209")
        return "Back of Backpack, Straps, and Grab Handle Color";
  ...
  const reorderSectionsForModel = (
    sections: MaterialSection[],
  ): MaterialSection[] => {
    if (modelParam.includes("Baseball-Jersey.glb")) {
      const reordered = [...sections];
      const frontIndex = reordered.findIndex(
        (s) => s.originalName === "Body_F_3732324",
      );
      const backIndex = reordered.findIndex(
        (s) => s.originalName === "Body_B_3794820",
      );

      if (frontIndex !== -1 && backIndex !== -1 && frontIndex > backIndex) {
        // Swap front and back so front comes first
        [reordered[frontIndex], reordered[backIndex]] = [
          reordered[backIndex],
          reordered[frontIndex],
        ];
      }
      return reordered;
    }
