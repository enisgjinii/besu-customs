...
export function RosterInput({
  ...
  const updatePlayer = useCallback(
    (id: string, updates: Partial<PlayerEntry>) => {
      onChange({
        ...safeValue,
  ...
  const updatePlayerSize = useCallback(
    (id: string, sizeType: "top" | "shorts", size: string) => {
      onChange({
        ...safeValue,
      ...
      {value.players.length === 0 ? (
        <div className="text-center py-8 border border-dashed rounded-lg">
          ...
          <p className="text-xs text-muted-foreground/70">
            Click "Add Player" to start building your roster
          </p>
        </div>
      ) : (
            ...
            >
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updatePlayer(player.id, {
                      nameOnJersey: e.target.value.toUpperCase(),
                    })
                  }
              ...
              <div className="col-span-2">
                <Select
                  ...
                  onValueChange={(size) =>
                    updatePlayerSize(player.id, "top", size)
                  }
                >
              ...
              <div className="col-span-2">
                <Select
                  ...
                  onValueChange={(size) =>
                    updatePlayerSize(player.id, "shorts", size)
                  }
                >
                  <SelectTrigger className="h-8 text-xs">
                    <SelectValue />
      ...
      {value.players.length > 0 && (
        <div className="p-3 bg-muted/50 rounded-lg border">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {SIZES.map((size) => {
              const topCount = value.players.filter(
                (p) => p.sizes.top === size,
              ).length;
              const shortsCount = value.players.filter(
                (p) => p.sizes.shorts === size,
              ).length;
