...
import { Activity, Clock, Users, ShoppingCart, FileText, Search } from "lucide-react";
              ...
              <div key={activity.id} className="flex items-start gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <p className="text-sm">
                    ...
                    {activity.type === 'purchase' && `Purchased ${activity.target} ($${activity.details?.amount})`}
                    {activity.type === 'download' && `Downloaded ${activity.target}`}
                    {activity.type === 'search' && `Searched for "${activity.details?.query}" (${activity.details?.results} results)`}
                  </p>
                </div>
              </div>
            ))}
