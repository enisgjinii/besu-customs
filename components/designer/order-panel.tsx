      ...
      {mode === "roster" ? (
        <>
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="shrink-0 text-[12px] font-semibold tabular-nums">{total} pcs</span>
          </div>

          {s.roster.length > 0 ? (
            <div className="min-w-0 rounded-xl bg-[#f7f7f5] px-3 py-2.5 ring-1 ring-border/55">
              <p className="m-0 text-[11px] font-semibold uppercase tracking-wider text-muted">Preview player</p>
              <p className="m-0 mt-0.5 text-[11px] leading-snug text-muted">Choose a roster entry to preview its customization data.</p>
                ...
                {s.roster.map((player) => {
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-10 shrink-0 snap-start rounded-full px-3 text-[11px] font-semibold ring-1 ${
                        selected
              ...
              {getPreviewPlayer(s) ? (
                <p className="m-0 mt-1.5 truncate text-[11px] text-muted">
                  Selected: <span className="font-semibold text-foreground">{getPreviewPlayer(s)?.name || "Unnamed"} #{getPreviewPlayer(s)?.number || "—"}</span>
                </p>
              ) : null}
            </div>
          ) : null}
          ...
          {s.roster.length === 0 ? (
            <div className="rounded-xl bg-[#f7f7f5] px-3 py-4 text-center ring-1 ring-border/60">
              ...
              <p className="m-0 mt-1 text-[11px] leading-snug text-muted">Add names, numbers, and sizes for each uniform.</p>
            </div>
          ) : null}

          <div className="flex min-w-0 flex-col gap-2">
            {s.roster.map((player, index) => (
              <div key={player.id} className="min-w-0 rounded-xl border border-border/80 bg-white p-3 shadow-[0_1px_0_rgba(15,23,42,0.03)]">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className="size-10 min-w-10 shrink-0 text-muted"
        ...
        <>
          <p className="m-0 text-[11px] font-semibold uppercase tracking-wider text-muted">Production files</p>
          <div className="grid grid-cols-2 gap-2 max-[340px]:grid-cols-1">
            <Button size="sm" variant="outline" className="min-h-11" isDisabled={Boolean(exporting) || !s.artwork.front || !s.artwork.back} onPress={() => runExport("png")}>
              {exporting === "png" ? <Spinner size="sm" /> : <Download className="size-3.5" />} PNGs
            </Button>
            <Button size="sm" variant="outline" className="min-h-11" isDisabled={Boolean(exporting) || !s.artwork.front || !s.artwork.back} onPress={() => runExport("svg")}>SVGs</Button>
            <Button size="sm" variant="outline" className="min-h-11" isDisabled={Boolean(exporting) || !s.artwork.front || !s.artwork.back} onPress={() => runExport("pdf")}>
              {exporting === "pdf" ? <Spinner size="sm" /> : "PDF"}
            </Button>
            <Button size="sm" className="min-h-11" isDisabled={Boolean(exporting) || !s.artwork.front || !s.artwork.back} onPress={() => runExport("zip")}>
              {exporting === "zip" ? <Spinner size="sm" color="current" /> : <Package className="size-3.5" />} ZIP
            </Button>
          </div>
          ...
          <Button fullWidth size="sm" className="min-h-12 font-semibold" isDisabled={errors.length > 0} onPress={checkout}>
            <ShoppingBag className="size-4" /> Add to Shopify
          </Button>
        </>
      ) : (
        <>
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="shrink-0 text-[12px] font-semibold">{errors.length ? `${errors.length} to do` : "Ready"}</span>
          </div>

          <div className="overflow-hidden rounded-xl border border-border/80 bg-white">
            {checks.map((check, index) => (
              <div key={check.label} className={`flex min-h-11 items-center gap-2 px-3 py-2 ${index ? "border-t border-separator" : ""}`}>
                {check.ready ? <CheckCircle2 className="size-4 shrink-0 text-success" /> : <Circle className="size-4 shrink-0 text-muted" />}
                <span className="min-w-0 flex-1 truncate text-[12px] font-semibold">{check.label}</span>
                <span className="shrink-0 text-[11px] text-muted">{check.ready ? "Ready" : "Needed"}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-1.5 rounded-xl bg-[#f7f7f5] px-3 py-2.5 ring-1 ring-border/50">
            <div className="flex min-w-0 justify-between gap-3">
              <span className="shrink-0 text-[12px] text-muted">Team</span>
              <span className="min-w-0 truncate text-right text-[12px] font-semibold">{s.teamName || "—"}</span>
            </div>
            <div className="flex min-w-0 justify-between gap-3">
              <span className="shrink-0 text-[12px] text-muted">Uniform</span>
              <span className="min-w-0 text-right text-[12px] font-semibold capitalize">{s.sport} · {s.garmentType}</span>
            </div>
          ...
          <Separator />
          <p className="m-0 text-[11px] font-semibold uppercase tracking-wider text-muted">Customer</p>
          <div className="flex min-w-0 flex-col gap-2">
            <TextField fullWidth className="min-w-0" name="customer-name" value={s.customer.name} onChange={(value) => s.patch({ customer: { ...s.customer, name: value } })}>
              <Label>Full name</Label><Input className="min-h-11" autoComplete="name" />
            </TextField>
            ...
            <TextField fullWidth className="min-w-0" name="customer-notes" value={s.customer.notes} onChange={(value) => s.patch({ customer: { ...s.customer, notes: value } })}>
              <Label>Notes</Label><TextArea rows={3} className="min-h-[96px]" />
            </TextField>
          </div>
        </>
      )}
